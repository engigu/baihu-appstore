...
------------------------------------------------------------------
作用:
  1. 自动扫描 apps/ 下的所有应用目录；
  2. 发现 build.json 时，自动调度对应的构建器（支持 Python、Node.js、Shell 等任意语言）；
  3. 验证构建产物 app.yaml；
  4. 自动扫描所有 app.yaml 并聚合生成全商店索引文件 apps.json。
...
def parse_yaml_metadata(yaml_path: Path) -> dict:
    """简易纯文本提取 app.yaml 中的核心元数据（避免依赖第三方 yaml 库）"""
    meta = {}
    try:
    ...
    if tasks_match:
        ...
        tasks = re.findall(r'^\s*-\s*id:\s*["\']?([a-zA-Z0-9_-]+)["\']?', tasks_block, re.MULTILINE)
        meta["tasks_count"] = len(tasks)
    else:
        meta["tasks_count"] = 0

    # 统计场景数（scenarios 下的 - id:）
    scenarios_match = re.search(r'scenarios:\s*\n(.*)', content, re.DOTALL)
    if scenarios_match:
        scenarios_block = scenarios_match.group(1)
        scenarios = re.findall(r'^\s*-\s*id:\s*["\']?([a-zA-Z0-9_-]+)["\']?', scenarios_block, re.MULTILINE)
        meta["scenarios_count"] = len(scenarios)
    else:
        meta["scenarios_count"] = 0
...
def main():
    ...
    app_dirs = [p for p in sorted(apps_root.iterdir()) if p.is_dir()]
    if not app_dirs:
